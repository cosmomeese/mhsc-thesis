### Main Function to merge all new Fitbit data together
...
data.Importer.CodeVersion <- "1.2"
extraMetrics.CodeVersion <- "1.1"
...
sourceDir <- "fitbit"
fcns <- list("fitbitPullScriptForStudy")

sourceDir2 <- "userDefinedFcns"
fcns2 <- list("importExcelSheets",
             "importSimonMetaData",
             "importSimonStepData",
             "importSimonCPSData",
...
invisible(lapply(fcns2,sourceDir2,
                 sourceDir=sourceDir)) #use function
rm(fcns2, sourceDir2)
...
stepData <- pullFitbitData(rDataFilePath=RDATA_SAVE_FILEPATH,
                           addFromCSVFilePath=CSV_FILEPATH)
